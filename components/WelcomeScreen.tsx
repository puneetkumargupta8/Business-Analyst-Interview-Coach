...
interface WelcomeScreenProps {
  onStart: (difficulty: Difficulty, domain: Domain, interviewType: InterviewType) => void;
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ 
  ...
  const handleStart = () => {
    onStart(selectedDifficulty, selectedDomain, selectedInterviewType);
  };

  return (
    <div className="w-full max-w-3xl mx-auto p-4 md:p-8 text-center animate-fade-in">
      ...
      <div className="mb-8 space-y-8">
        <div>
          <h2 className="text-2xl font-semibold text-white mb-4">1. Choose your difficulty</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {difficulties.map(({ name, icon, description }) => (
              <button
                key={name}
                onClick={() => setSelectedDifficulty(name)}
                className={`relative p-6 rounded-lg text-left transition-all duration-200 ${
                  selectedDifficulty === name
                    ? 'bg-brand-primary/20 border-2 border-brand-primary ring-2 ring-brand-primary/50'
                    : 'bg-neutral-800 border-2 border-neutral-700 hover:border-brand-primary'
                }`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <h2 className="text-2xl font-semibold text-white mb-4">2. Select an Industry</h2>
            <div className="relative">
              <select 
                  value={selectedDomain}
                  onChange={(e) => setSelectedDomain(e.target.value as Domain)}
                  className="w-full appearance-none bg-neutral-800 border-2 border-neutral-700 text-white text-lg rounded-lg p-3 focus:ring-2 focus:ring-brand-primary focus:outline-none focus:border-brand-primary"
                >
                  {domains.map(domain => (
          ...
          <div>
            <h2 className="text-2xl font-semibold text-white mb-4">3. Choose Interview Type</h2>
            <div className="relative">
              <select 
      ...
      <button
        onClick={handleStart}
        className="bg-brand-primary hover:bg-brand-primary/80 text-neutral-900 font-bold py-3 px-8 rounded-lg text-lg transition-transform duration-200 transform hover:scale-105 mt-4"
      >
        Start Interview
      </button>
    </div>
